...
export const registerUser = expressAsyncHandler(async (req: Request, res: Response) => {
  ...
  if (!userService.validUserModel(userData)) {
    res.status(400);
    throw new Error(
      `Please add all fields. Name: ${userData.name ? "Valid" : "Invalid"}, Email: ${
        userData.email ? "Valid" : "Invalid"
      }, Password: ${userData.password ? "Valid" : "Invalid"}`
    );
  }
...
export const loginUser = expressAsyncHandler(async (req: Request, res: Response) => {
  ...
  const  token = userService.generateJWT(userDb);
